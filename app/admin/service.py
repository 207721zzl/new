...
class AdminService:
    ...
    def __init__(
        ...
        *,
        knowledge_store_factory: Callable[[], MilvusKnowledgeStore]
        | None = None,
    ) -> None:
        self.session = session
        self.knowledge_store_factory = knowledge_store_factory or MilvusKnowledgeStore
    ...
    def _delete_vectors(self, document_id: str) -> None:
        store = self.knowledge_store_factory()
        try:
            if store.client.has_collection(store.settings.milvus_collection):
    ...
    async def delete_knowledge_document(
        ...
        *,
        ...
        parent_count = int(
            await self.session.scalar(
                select(func.count())
                .select_from(KnowledgeParentChunkRecord)
                .where(KnowledgeParentChunkRecord.document_id == document_id)
            )
            or 0
        )
        child_count = int(
        ...
        try:
            ...
            self._add_audit(
                actor_user_id=actor_user_id,
                action="admin.knowledge_document_deleted",
            ...
            await self.session.rollback()
            logger.exception("knowledge document deletion failed document_id=%s", document_id)
            raise KnowledgeDeletionError() from exc
